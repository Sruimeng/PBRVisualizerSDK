...
export class MaterialSystem {
  ...
  public applyMaterialUpdates(object: THREE.Object3D, materialId?: string): void {
    ...
    if (!config) {
      console.warn(
        `[MaterialSystem] No material config found for id: ${materialId}. Available configs:`,
        Array.from(this.materialConfigs.keys()),
      );
      return;
    }
  ...
  public createPresetMaterial(
    type: 'metal' | 'plastic' | 'wood' | 'glass' | 'fabric',
  ): MaterialState {
    switch (type) {
      case 'metal':
        return {
  ...
  public cleanup(): void {
    ...
    this.materialCache.forEach((material) => {
      ...
      [stdMaterial.map, stdMaterial.normalMap, stdMaterial.aoMap, stdMaterial.emissiveMap].forEach(
        (texture) => {
