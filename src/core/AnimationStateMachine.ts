...
import { TransitionEffectType } from '../types';
...
export class AnimationStateMachine {
  ...
  public trigger(transitionId: string): boolean {
    if (this.isTransitioning) {
      if (this.config.debug) {
        console.warn(
          '[AnimationStateMachine] Cannot trigger transition while already transitioning',
        );
      }
      return false;
    }
    ...
    if (!transition) {
      if (this.config.debug) {
        console.warn(
          `[AnimationStateMachine] Transition '${transitionId}' not found from state '${this.currentStateId}'`,
        );
      }
      return false;
    }
  ...
  private startTransition(transition: StateTransition): void {
    ...
    if (this.config.debug) {
      console.log(
        `[AnimationStateMachine] Transition started: ${transition.from} -> ${transition.to}`,
      );
      console.log(
        `[AnimationStateMachine] Effect type: ${this.currentEffect.type}, duration: ${this.currentEffect.duration}ms`,
      );
    }
  }
  ...
  private pauseCurrentAnimation(fromState: StateMachineState | undefined): void {
    ...
    const animationName =
      fromState.animationName ||
      (fromState.animationIndex !== undefined
        ? this.animations[fromState.animationIndex]?.name
        : undefined);
    ...
    if (animationName) {
      ...
      if (action && action.isRunning()) {
        ...
        if (this.config.debug) {
          console.log(
            `[AnimationStateMachine] Paused animation: ${animationName} at time ${this.pausedActionTime}`,
          );
        }
      }
    }
  ...
  private switchToTargetAnimation(): void {
    ...
    if (toState) {
      const targetAnimName =
        ...
        (toState.animationIndex !== undefined
          ? this.animations[toState.animationIndex]?.name
          : undefined);
  ...
  private resumePausedAnimation(): void {
    if (this.pausedAction) {
      ...
      if (this.config.debug) {
        console.log(
          `[AnimationStateMachine] Resumed paused animation at time ${this.pausedActionTime}`,
        );
      }
    }
  }
  ...
  private applyTransitionEffect(progress: number): void {
    ...
    if (effectType === TransitionEffectType.Fade || effectType === TransitionEffectType.FadeScale) {
      ...
      if (
        this.config.debug &&
        (progress < 0.02 || (progress > 0.48 && progress < 0.52) || progress > 0.98)
      ) {
        console.log(
          `[AnimationStateMachine] Opacity: ${opacity.toFixed(2)} at progress ${progress.toFixed(2)}`,
        );
      }
    }
    ...
    if (
      ...
      effectType === TransitionEffectType.FadeScale
    ) {
      ...
      if (
        ...
        console.log(
          `[AnimationStateMachine] Scale: ${scale.toFixed(2)} at progress ${progress.toFixed(2)}`,
        );
      }
    }
  }
  ...
  private completeTransition(): void {
    ...
    if (this.config.debug) {
      console.log(
        `[AnimationStateMachine] Transition completed: ${this.previousStateId} -> ${toStateId}`,
      );
    }
